...
bool EmergencyService::changeResources(int delta)
{
    ...
    {
        TheArk::get_instance()->getResources()->setUsedToJunk(- delta, Navigation_Service);  //вернули мусор
    }
    else //добавляем ресурсы в количестве недостающих - 10 - ежегодный износ
    {
        TheArk::get_instance()->getResources()->setComponentsToUsed(delta, Navigation_Service);
    }
